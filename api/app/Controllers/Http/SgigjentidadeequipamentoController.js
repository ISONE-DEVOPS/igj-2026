...
const controller = "Sgigjentidadeequipamento";
  ...
  async index({ request, response }) {
    ...
    if (allowedMethod) {
      ...
      var result = await Model
        ...
        .with('sgigjentidade')
        .with('sgigjprequipamentotp')
        .with('sgigjprequipamentoclassificacao')
        .where(data)
        .where('ESTADO', 1)
    ...
    if (allowedMethod || true) {
      ...
      const content = `<div >
        <div style="margin:20px 20px 20px 40px;width: 90%;z-index: 2;">
            ...
            <h2 style="font-size: 12pt !important">Equipamentos</h2>
        ...
        <td  style="text-align: center;"> ${element.sgigjprequipamentotp.DESIG}</tb>
        <td  style="text-align: center;"> ${element.sgigjprequipamentoclassificacao.CLASSIFICACAO}</tb>
        <td  style="text-align: center;"> ${element.DESIG}</tb>
        <td  style="text-align: right;"> ${this.formatCurrency(element.PRECO)}</tb>
        <td  style="text-align: center;"> ${element.ANO}</tb>
        <td  style="text-align: center;"> ${moment(element.DT_AQUISICAO).format('DD-MM-Y')}</tb>
        <td  style="text-align: center;"> ${element.REFERENCIA}</tb>
        </tr>`
          }
          return tbody
        })()}
      ...
      response.header('Content-Disposition', 'attachment; filename="equipamento.pdf"')
    ...
    if (allowedMethod || true) {
      ...
      for (let index = 0; index < result.length; index++) {
        ...
        dataResult.push({
          ...
          "Tipo de Equipamento": element.sgigjprequipamentotp.DESIG,
          "Classificação": element.sgigjprequipamentoclassificacao.CLASSIFICACAO,
          "Designação": element.DESCR,
          "Preço": element.PRECO,
          "Ano": element.ANO,
          "Data Aquisição": moment(element.DT_AQUISICAO).format('DD-MM-Y'),
      ...
      response.header('Content-Disposition', 'attachment; filename="equipamento.csv"')
