...
// Perfis de sistema que não podem ser eliminados (configurável)
const PROTECTED_PROFILE_IDS = [
  "c55fc99dc15b5f5e22abb36d3eb393db4082",  // Administrador
  "f8382845e6dad3fb2d2e14aa45b14f0f85de",  // Perfil de sistema
];
...
data.CRIADO_POR = request.userID
  ...
  async destroy ({ params,  response, request}) {
    ...
    if(allowedMethod){
      ...
      if(PROTECTED_PROFILE_IDS.includes(params.id)) return response.status(403).json({status:"erro",entity:table,message:"this profile is protected and cannot be deleted",code:246})
 ...
 "DELETADO_POR":request.userID,         "DELETADO_EM":functionsDatabase.createDateNow(table)
  ...
  async index ({ request, response }) {
    ...
    if(allowedMethod){
      ...
      var result = await Model.query().orderBy('DT_REGISTO','desc').where(data).where('ESTADO', 1).fetch()
