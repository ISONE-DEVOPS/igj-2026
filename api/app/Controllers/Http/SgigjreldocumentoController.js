...
const controller = "Sgigjreldocumento";
...
class entity {
  ...
  async destroy({ params, response, request }) {
    ...
    if (allowedMethod) {
      ...
      let documento = await Model.query().where('ID', '' + params.id).first()

      if (documento && documento.DOC_URL) {
        const filePath = documento.DOC_URL.replace("https://firebasestorage.googleapis.com/v0/b/igj-sgigj.firebasestorage.app/o/", '');
        // return filePath
        const arquivoRef = storage.bucket().file(filePath);
        arquivoRef.delete()
          .then(() => {
  ...
  async index({ request, response }) {
    ...
    if (allowedMethod) {
      ...
      var result = await Model.query().where(data).where('ESTADO', 1).orderBy('DT_REGISTO', 'desc').fetch()
