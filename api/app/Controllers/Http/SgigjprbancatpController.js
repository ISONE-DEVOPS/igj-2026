...
const controller = "Sgigjprbancatp";
...
class entity {
  ...
  async index({ request, response }) {
    ...
    if (allowedMethod) {
      ...
      var result = await Model.query().with('criadoPor.sgigjrelpessoaentidade.sgigjpessoa').where(data).orderBy('DT_REGISTO', 'desc').where('ESTADO', 1).fetch()
