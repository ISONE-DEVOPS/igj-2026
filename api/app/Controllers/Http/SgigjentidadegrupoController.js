...
const controller = "Sgigjentidadegrupo";
...
class entity {
    ...
    if (allowedMethod || true) {
      ...
      const content = `<div >
        <div style="margin:20px 20px 20px 40px;width: 90%;z-index: 2;">
            ...
            <h2 style="font-size: 12pt !important">Grupos de Paramentrizações</h2>
        ...
        <td > ${element.TIPO}</tb>
        <td > ${element.DESCR}</tb>
        <td > ${moment(element.DT_REGISTO).format('DD-MM-Y')}</tb>
        </tr>`
          }
          return tbody
        })()}
      ...
      response.header('Content-Disposition', 'attachment; filename="grupo.pdf"')
  ...
  async exportCsv({ request, response }) {
    ...
    if (allowedMethod || true) {
      ...
      response.header('Content-Disposition', 'attachment; filename="grupo.csv"')
