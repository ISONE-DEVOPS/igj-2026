...
const Model = use('App/Models/DecisaoTutelarProcessos');
...
class DecisaoTutelarProcessosController extends GenericController {

    table = "decisao_tutelar_processos";
    Model = Model
            ...
            if (validation.status === 'ok') {
                ...
                if (newE[0] === 0) {
                    ...
                    for (let index = 0; index < documentos.length; index++) {
                        ...
                        element.DECISAO_TUTELAR_PROCESSOS_ID = data.ID
                    ...
                    // Notificar perfis sobre recurso à Tutela
                    try {
                        ...
                        GlbnotificacaoFunctions.storeToPerfil({
                            ...
                            MSG: `Recurso do Processo (Código: ${codigoProcesso}) à Tutela, por ${nomeUtilizador} em ${dataHoje}.`,
                            TITULO: "Recurso à Tutela",
                            PESSOA_ID: null,
                        ...
                        console.error('Erro ao enviar notificação de recurso tutela:', err);
                    }
                    ...
                    if (newE === 1) {
                        ...
                        if (documentos) {
                            ...
                            const deleteDocuments = await DatabaseDB
                                .table(this.tableDocument)
                                .where('DECISAO_TUTELAR_PROCESSOS_ID', '' + params.id)
                                .userID(request.userID)
                                .delete()
                            ...
                            for (let index = 0; index < documentos.length; index++) {
                                let element = documentos[index];
                                element.DECISAO_TUTELAR_PROCESSOS_ID = params.id
                                element.ID = await functionsDatabase.createID(this.tableDocument)
                                element.DT_REGISTO = functionsDatabase.createDateNow(this.table)
                                element.ESTADO = 1
                                const validationDocuments = await functionsDatabase.validation(listDocuments, element, extractRequestDocument, this.tableDocument);
...
module.exports = DecisaoTutelarProcessosController