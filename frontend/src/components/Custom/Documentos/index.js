...
const Documentos = ({ documentolist, list, save, item, onSendData }) => {
    ...
    async function savedata() {
        ...
        if (docedit == true) {
            ...
            const response2x = await api.delete('/sgigjreldocumento/0?' + item?.ENTIDADE + '=' + item?.ID);
    ...
    useEffect(() => {
        ...
        if (list.length == 0) {
            ...
            for (let ix = 0; ix < list.length; ix++) {
                ...
                const DT_E = list[ix].DT_EMISSAO != null ? list[ix].DT_EMISSAO.substring(0, 10) : ""
                const DT_V = list[ix].DT_VALIDADE != null ? list[ix].DT_VALIDADE.substring(0, 10) : ""
