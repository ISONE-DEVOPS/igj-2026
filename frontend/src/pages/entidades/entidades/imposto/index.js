...
function Table({ columns, data, modalOpen, years, uploadList, uploadmeiospagamento }) {
    ...
    var total_imposto
    var total_orcamento
    var total_fundo_area_cobertura
    var total_fundo_ensino
    var total_fundo_cultura
    var total_fundo_desporto
    var total_fundo_turismo
    console.log(data)
    ...
    if (data.length > 0) {
        ...
        const listAreaCobertura = data.map(res => parseCurrency(res.FUNDO_AREA_COBERTURA))
        total_fundo_area_cobertura = sumValues(listAreaCobertura)

        const listEnsino = data.map(res => parseCurrency(res.FUNDO_ENSINO))
        total_fundo_ensino = sumValues(listEnsino)

        const listCultura = data.map(res => parseCurrency(res.FUNDO_CULTURA))
        total_fundo_cultura = sumValues(listCultura)

        const listDesporto = data.map(res => parseCurrency(res.FUNDO_DESPORTO))
        total_fundo_desporto = sumValues(listDesporto)

        const listTurismo = data.map(res => parseCurrency(res.FUNDO_TURISMO))
        total_fundo_turismo = sumValues(listTurismo)
        const listOrcamento = data.map(res => parseCurrency(res.ORCAMENTO_ESTADO))
        total_orcamento = sumValues(listOrcamento)
    ...
    async function exportPDFImposto() {
        ...
        try {
            ...
            const response = await api.get(`/export-pdf/imposto?ENTIDADE_ID=${id}` + setParams([["ANO", ano]]), { responseType: "blob" });
            ...
            <BTable striped bordered hover responsive {...getTableProps()}>
                ...
                <tbody {...getTableBodyProps()}>
                    ...
                    <tr>
                        ...
                        <td className='font-weight-bold text-right text-right'>{formatCurrency(total_orcamento)}</td>
                        <td className='font-weight-bold text-right'>{formatCurrency(total_fundo_turismo)}</td>
                        <td className='font-weight-bold text-right'>{formatCurrency(total_fundo_desporto)}</td>
                        <td className='font-weight-bold text-right'>{formatCurrency(total_fundo_cultura)}</td>
                        <td className='font-weight-bold text-right'>{formatCurrency(total_fundo_area_cobertura)}</td>
                        <td className='font-weight-bold text-right'>{formatCurrency(total_fundo_ensino)}</td>
...
const Imposto = () => {
    ...
    const columns = React.useMemo(
        () => [
            ...
            {
                Header: 'Orçamento Estado',
                accessor: 'ORCAMENTO_ESTADO',
                centered: false
            },
            {
                Header: 'Fundo Des. Turismo',
                accessor: 'FUNDO_TURISMO',
                centered: false
            }, {
                Header: 'Fundo Des. Desporto',
                accessor: 'FUNDO_DESPORTO',
                centered: false
            },
            {
                Header: 'Fundo A. A. Cultura',
                accessor: 'FUNDO_CULTURA',
                centered: false
            },
            {
                Header: 'Mun. Área Coberta Concessão',
                accessor: 'FUNDO_AREA_COBERTURA',
                centered: false
            },
            {
                Header: 'Fundo A.Ensino e Formação',
                accessor: 'FUNDO_ENSINO',
                centered: false
            },
            {
                Header: 'Ações',
    ...
    async function uploadlist(ano) {
        ...
        try {
            ...
            const response = await api.get('/imposto?ENTIDADE_ID=' + id + setParams([["ANO", ano]]));

            if (response.status == '200') {

                for (var i = 0; i < response.data.length; i++) {
                    const idx = response.data[i].ID
                    response.data[i].IMPOSTO = formatCurrency(response.data[i].IMPOSTO)
                    console.log(response.data[i].IMPOSTO)
                    response.data[i].FUNDO_TURISMO = formatCurrency(response.data[i].FUNDO_TURISMO)
                    response.data[i].FUNDO_DESPORTO = formatCurrency(response.data[i].FUNDO_DESPORTO)
                    response.data[i].FUNDO_ENSINO = formatCurrency(response.data[i].FUNDO_ENSINO)
                    response.data[i].FUNDO_CULTURA = formatCurrency(response.data[i].FUNDO_CULTURA)
                    response.data[i].FUNDO_AREA_COBERTURA = formatCurrency(response.data[i].FUNDO_AREA_COBERTURA)
                    response.data[i].BRUTO = formatCurrency(response.data[i].BRUTO)
                    response.data[i].ORCAMENTO_ESTADO = formatCurrency(response.data[i].ORCAMENTO_ESTADO)
                    ...
                    if (response.data[i].pagamento.length > 0) {
                        ...
                        response.data[i].DT_CONFIRMACAO = formatDate(response.data[i].pagamento[0].DT_CONFIRMACAO)
    ...
    async function editarItemGO(event) {
        ...
        try {
            ...
            const response = await api.put('/imposto/' + itemSelected.ID, upload);
    ...
    async function criarRecibimentoGO(event) {
        ...
        const upload = {
            ...
            VALOR: parseCurrency(itemSelected.BRUTO),
            NUM_DOC: cheque,
        ...
        try {
            ...
            const response = await api.post('/payment/imposto', upload);
    ...
    async function criarItemGO(event) {
        ...
        try {
            ...
            const response = await api.post('/imposto', upload);
    ...
    const removeItemFunction = async (idx) => {
        ...
        try {
            ...
            const response = await api.delete('/imposto/' + idx);
                                ...
                                <Row>
                                    ...
                                    <Col sm={6}>
                                        <div className="form-group fill">
                                            ...
                                            <input type="number" onChange={event => { setBRUTO(event.target.value) }} defaultValue={itemSelected.BRUTO} className="form-control" placeholder="Receita Bruta..." required />
                            ...
                            <form id="editarItem"  >
                                ...
                                <Row>
                                    ...
                                    <Col sm={6}>
                                        <div className="form-group fill">
                                            <label className="floating-label" htmlFor="Name">Receita Bruta <span style={{ color: "red" }} >*</span></label>
                                            <input readOnly="readOnly" type="text" onChange={event => { setBRUTO(event.target.value) }} defaultValue={itemSelected.BRUTO} className="form-control" placeholder="Receita Bruta ..." required />
                                    ...
                                    <Col sm={6}>
                                        <div className="form-group fill">
                                            ...
                                            <input readOnly="readOnly" defaultValue={itemSelected.DUC} type="number" onChange={event => { setDUC(event.target.value) }} className="form-control" placeholder="Número do DUC..." required />
                            ...
                            <Modal.Body className="newuserbox" style={activeProfileTab === "recebimento" ? {} : { display: "none" }} >
                                <form id="criarItem"  >
                                    ...
                                    <Row>
                                        ...
                                        <Col md={6}>
                                            <div className="form-group fill">
                                                ...
                                                <input
                                                    type="text"
                                                    readOnly="readOnly"
                                                    defaultValue={formatCurrency(itemSelected.pagamento[0].VALOR)}
                                                    onChange={(event) => {
                                        ...
                                        <Col sm={12}>
                                            <div className="form-group fill">
                                                {itemSelected.pagamento[0].sgigjreldocumento && itemSelected.pagamento[0].sgigjreldocumento.length > 0 ? (
