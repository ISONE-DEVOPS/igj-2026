...
import { pageEnable, taskEnable, taskEnableIcon, taskEnableTitle, formatCurrency, parseCurrency, sumValues, setParams } from '../../../../functions';
...
function Table({ columns, data, modalOpen, years, uploadList, uploadmeiospagamento }) {
    ...
    const { id } = useParams();
    var total_Bruto
    var totalArt49Percent
    var totalArt48Percent

    var totalRecebimento
    if (data.length > 0) {
        const listBruto = data.map(res => parseCurrency(res.BRUTO))
        const listArt48Percent = data.map(res => parseCurrency(res.Art_48_percent))
        const listArt49Percent = data.map(res => parseCurrency(res.Art_49_percent))
        // const listArt48 = data.map(res => parseCurrency(res.Art_48))
        // const listArt49 = data.map(res => parseCurrency(res.Art_49))
        const listRecebimento = data.map(res => parseCurrency(res.TOTAL))

        total_Bruto = sumValues(listBruto)
        totalArt49Percent = sumValues(listArt48Percent)
        totalArt48Percent = sumValues(listArt49Percent)
        // totalArt49 = sumValues(listArt49)
        // totalArt48 = sumValues(listArt48)
        totalRecebimento = sumValues(listRecebimento)
    }
    ...
    function optionDownload(value) {
        if (value === "1") {
            exportPDFContrapartida();
        } else if (value === "2") {
            exportExel()
        }
    }
    async function exportPDFContrapartida() {
        const ano = ANO === "" ? undefined : ANO

        try {

            const response = await api.get(`/export-pdf/contrapartida?ENTIDADE_ID=${id}` + setParams([["ANO", ano]]), { responseType: "blob" });
    ...
    async function exportExel() {
        ...
        try {
            ...
            const response = await api.get(`/export-csv/contrapartida?ENTIDADE_ID=${id}` + setParams([["ANO", ano]]), { responseType: "blob" });
            ...
            <BTable striped bordered hover responsive {...getTableProps()}>
                    ...
                    {headerGroups.map(headerGroup => (
                        <tr {...headerGroup.getHeaderGroupProps()}>
                            {headerGroup.headers.map(column => (
                                ...
                                <th className={column.id === 'TOTAL' || column.id === 'BRUTO' || column.id === 'Art_49_percent' || column.id === 'Art_48_percent' ? 'text-right' : ''} {...column.getHeaderProps(column.getSortByToggleProps())}>
                ...
                <tbody {...getTableBodyProps()}>
                    ...
                    <tr>
                        ...
                        <td className='font-weight-bold text-right'>{formatCurrency(totalArt48Percent)}</td>
                        {/* <td className='font-weight-bold'>{formatCurrency(totalArt48)}</td> */}
                        <td className='font-weight-bold text-right'>{formatCurrency(totalArt49Percent)}</td>
                        {/* <td className='font-weight-bold'>{formatCurrency(totalArt49)}</td> */}
                        <td className='font-weight-bold text-right'>{formatCurrency(totalRecebimento)}</td>
                        <td></td>
...
const Contrapartida = () => {
    ...
    const [Art_49, setArt_49] = useState(0);
    const [ENTIDADE_ID, setENTIDADE_ID] = useState("");
    const [DUC, setDUC] = useState(0);
    const [DT_EMISSAO_DUC, setDT_EMISSAO_DUC] = useState(0);

    const [DUC1, setDUC1] = useState(0);
    const [DUC2, setDUC2] = useState(0);
    const [DT_EMISSAO_DUC1, setDT_EMISSAO_DUC1] = useState(0);
    const [DT_EMISSAO_DUC2, setDT_EMISSAO_DUC2] = useState(0);
    ...
    const [contrapartida, setContrapartida] = useState(false);
    ...
    const [activeProfileTab, setActiveProfileTab] = useState("receita");
    ...
    async function uploadlist(ano) {
        ...
        setid_params(id)
        try {
            const response = await api.get('/contrapartida?ENTIDADE_ID=' + id + setParams([["ANO", ano]]));

            if (response.status == '200') {

                for (var i = 0; i < response.data.length; i++) {
                    const idx = response.data[i].ID
                    response.data[i].VALOR_BRUTO = response.data[i].BRUTO
                    response.data[i].Art_48_percent = formatCurrency(response.data[i].Art_48_percent)
                    response.data[i].Art_49_percent = formatCurrency(response.data[i].Art_49_percent)
                    response.data[i].Art_48 = formatCurrency(response.data[i].Art_48)
                    response.data[i].Art_49 = formatCurrency(response.data[i].Art_49)
                    response.data[i].BRUTO = formatCurrency(response.data[i].BRUTO)

                    response.data[i].id = response.data[i].ID
                    response.data[i].TOTAL = formatCurrency((parseCurrency(response.data[i].Art_48_percent) + parseCurrency(response.data[i].Art_49_percent)))
                    ...
                    response.data[i].action =
                        <React.Fragment>
                            ...
                            {!response.data[i]?.pagamento?.length > 0 && taskEnable(pageAcess, permissoes, "Recebimento") == true ? <Link to='#' title={taskEnableTitle(pageAcess, permissoes, "Recebimento")} onClick={() => openCriarRecebimento(itemx)} className="text-primary mx-1"><i className={"text-primary " + taskEnableIcon(pageAcess, permissoes, "Recebimento")} /></Link>
                                : null
                            }
    ...
    const openVerHandler = (idx) => {
        ...
        setArt_48(idx.Art_48_percent)
        setArt_49(idx.Art_49_percent)
        setVerOpen(true);
    ...
    const openEditHandler = (idx) => {
        ...
        // idx.BRUTO = parseCurrency(idx.BRUTO)
        idx.Art_48_percent = parseCurrency(idx.Art_48_percent)
        idx.Art_49_percent = parseCurrency(idx.Art_49_percent)
        setIsEditarRecebimentoOpen(false);
        setIsOpen(false);
        setVerOpen(false);
        setANO(idx.ANO)
        setBRUTO(idx.VALOR_BRUTO)
        setDUC1(parseInt(idx.DUC_Art_48))
        setDUC2(parseInt(idx.DUC_Art_49))
        setDT_EMISSAO_DUC1(createDate2(idx.DT_EMISSAO_DUC_Art_48))
        setDT_EMISSAO_DUC2(createDate2(idx.DT_EMISSAO_DUC_Art_49))
        setDUC(idx.DUC)
        setArt_48(idx.Art_48_percent)
        setArt_49(idx.Art_49_percent)
        setMES(idx.MES)
    ...
    async function editarItemGO(event) {
        ...
        const upload = {
            ...
            DUC_Art_48: DUC1,
            DUC_Art_49: DUC2,
            DT_EMISSAO_DUC_Art_48: DT_EMISSAO_DUC1,
            DT_EMISSAO_DUC_Art_49: DT_EMISSAO_DUC2
        }
        ...
        try {
            ...
            const response = await api.put('/contrapartida/' + itemSelected.ID, upload);
    ...
    async function criarRecibimentoGO(event) {
        ...
        const upload = {
            DUC: DUC,
            DT_EMISSAO_DUC: DT_EMISSAO_DUC,
            DUC_2: DUC2,
            DT_EMISSAO_DUC_2: DT_EMISSAO_DUC2,
            VALOR: itemSelected.BRUTO,
            NUM_DOC: cheque,
            DT_CONFIRMACAO: dataconfirmacao,
            contrapartidas_ID: contrapartida,
            banco_iD: banco,
        ...
        try {
            ...
            const response = await api.post('/payment/contrapartida', upload);
    ...
    async function criarItemGO(event) {
        ...
        try {
            ...
            const response = await api.post('/contrapartida', upload);
    ...
    const removeItemFunction = async (idx) => {
        ...
        try {
            ...
            const response = await api.delete('/contrapartida/' + idx);
    ...
    var [novosdocumentos, setnovosdocumentos] = useState([
        {
            id: "" + Math.random(),
            "PR_DOCUMENTO_TP_ID": "02305a34c6621b511586cf800dd7b29ed43f",
            DOC_URL: null
        },
    ]);
    ...
    function removenovosdocumentos(i) {
            ...
            <Row>
                <Col sm={12}>
                        ...
                        <Modal.Body className="newuserbox" >
                            ...
                            <form id="criarItem" onSubmit={criarItemGO} >
                                ...
                                <Row>
                                        ...
                                        <div className="form-group fill">
                                            ...
                                            <span style={{ display: "flex", flexDirection: "row", alignItems: "center" }}>
                                                ...
                                                <input
                                                    ...
                                                    placeholder='Ano...'
                                                    onChange={event => { setANO(event.target.value) }} required />
                                            </span>
                                    ...
                                    <Col sm={6}>
                                        <div className="form-group fill">
                                            <label className="floating-label" htmlFor="Name">Número do DUC 1    <span style={{ color: "red" }} >*</span></label>

                                            <input type="number" onChange={event => { setDUC1(event.target.value) }} defaultValue={""} className="form-control" placeholder="Número do DUC ..." required />
                                    ...
                                    <Col sm={6}>
                                        <div className="form-group fill">
                                            <label className="floating-label" htmlFor="Name">Data Emissão DUC 1 <span style={{ color: "red" }} >*</span></label>
                                            <input
                                                onChange={(event) => {
                                                    setDT_EMISSAO_DUC1(event.target.value);
                                                }} type="date"
                                                className="form-control"
                                                placeholder="Data..."
                                                required
                                    ...
                                    <Col sm={6}>
                                        <div className="form-group fill">
                                            <label className="floating-label" htmlFor="Name">Número do DUC 2   <span style={{ color: "red" }} >*</span></label>

                                            <input type="number" onChange={event => { setDUC2(event.target.value) }} defaultValue={""} className="form-control" placeholder="Número do DUC ..." required />
                                    ...
                                    <Col sm={6}>
                                        <div className="form-group fill">
                                            <label className="floating-label" htmlFor="Name">Data Emissão DUC 2 <span style={{ color: "red" }} >*</span></label>
                                            <input
                                                onChange={(event) => {
                                                    setDT_EMISSAO_DUC2(event.target.value);
                                                }} type="date"
                                                className="form-control"
                                                placeholder="Data..."
                                                required
                                    ...
                                    <Col sm={12}>
                                        <div className="form-group fill">
                                            <label className="floating-label" htmlFor="Name">Receita Bruta $ <span style={{ color: "red" }} >*</span></label>

                                            <input type="number" onChange={event => { setBRUTO(event.target.value) }} defaultValue={""} className="form-control" placeholder="Receita Bruta..." required />

                                        </div>
                                    </Col>
                                    {/* <Col sm={6}>
                                        <div className="form-group fill">
                                            <label className="floating-label" htmlFor="Name">DUC (Art° 48)  <span style={{ color: "red" }} >*</span></label>

                                            <input type="number" onChange={event => { setArt_48(event.target.value) }} defaultValue={""} className="form-control" placeholder="DUC (Art° 48)..." required />

                                        </div>
                                    </Col>
                                    <Col sm={6}>
                                        <div className="form-group fill">
                                            <label className="floating-label" htmlFor="Name">DUC (Art° 49)  <span style={{ color: "red" }} >*</span></label>

                                            <input type="number" onChange={event => { setArt_49(event.target.value) }} defaultValue={""} className="form-control" placeholder="DUC (Art° 49)..." required />
                            ...
                            <form id="editarItem" onSubmit={editarItemGO} >
                                ...
                                <Row>
                                    <Col sm={12}>
                                        <div className="form-group fill">
                                            <label className="floating-label" htmlFor="Name">Código <span style={{ color: "red" }} >*</span></label>
                                            <input defaultValue={itemSelected.CODIGO} disabled type="text" className="form-control" id="Name" value="*" required />
                                        </div>
                                    </Col>
                                    ...
                                    <Col sm={6}>
                                        <div className="form-group fill">
                                            ...
                                            <input type="number" onChange={event => { setDUC1(event.target.value) }} defaultValue={parseInt(itemSelected.DUC_Art_48)} className="form-control" placeholder="Número do DUC ..." required />
                                    ...
                                    <Col sm={6}>
                                        <div className="form-group fill">
                                            <label className="floating-label" htmlFor="Name">Data Emissão DUC 1 <span style={{ color: "red" }} >*</span></label>
                                            <input
                                                defaultValue={createDate2(itemSelected.DT_EMISSAO_DUC_Art_48)}
                                                onChange={(event) => {
                                                    setDT_EMISSAO_DUC1(event.target.value);
                                                }} type="date"
                                                className="form-control"
                                    ...
                                    <Col sm={6}>
                                        <div className="form-group fill">
                                            ...
                                            <input type="number" onChange={event => { setDUC2(event.target.value) }} defaultValue={parseInt(itemSelected.DUC_Art_49)}
                                                className="form-control" placeholder="Número do DUC ..." required />
                                    ...
                                    <Col sm={6}>
                                        <div className="form-group fill">
                                            <label className="floating-label" htmlFor="Name">Data Emissão DUC 2 <span style={{ color: "red" }} >*</span></label>
                                            <input
                                                defaultValue={createDate2(itemSelected.DT_EMISSAO_DUC_Art_49)}
                                                onChange={(event) => {
                                                    setDT_EMISSAO_DUC2(event.target.value);
                                                }} type="date"
                                                className="form-control"
                                    ...
                                    <Col sm={12}>
                                        <div className="form-group fill">
                                            ...
                                            <input type="number" onChange={event => { setBRUTO(event.target.value) }} defaultValue={BRUTO} className="form-control" placeholder="Receita Bruta..." required />
                                    ...
                                    {/* <Col sm={6}>
                                        <div className="form-group fill">
                                            ...
                                            <input type="text" onChange={event => { setArt_48(event.target.value) }} defaultValue={itemSelected.Art_48_percent} className="form-control" placeholder="DUC (Art° 48)..." required />
                                    ...
                                    <Col sm={6}>
                                        <div className="form-group fill">
                                            <label className="floating-label" htmlFor="Name">DUC (Art° 49)  <span style={{ color: "red" }} >*</span></label>
                                            <input type="text" onChange={event => { setArt_49(event.target.value) }} defaultValue={itemSelected.Art_49_percent} className="form-control" placeholder="DUC (Art° 49)..." required />
                                        </div>
                                    </Col> */}
                                </Row>
                    ...
                    <Modal size='x' show={isVerOpen} onHide={() => setVerOpen(false)}>
                        ...
                        >
                            <li className="nav-item">
                                <Link
                                    to="#"
                                    className={
                                        activeProfileTab === "receita"
                                            ? profileTabActiveClass
                                            : profileTabClass
                                    }
                                    onClick={() => {
                                        setActiveProfileTab("receita");
                                    }}
                                    id="profile-tab"
                                >
                        ...
                        <Modal.Body className="newuserbox" style={activeProfileTab === "receita" ? {} : { display: "none" }} >
                                ...
                                <Row>
                                    ...
                                    <Col sm={6}>
                                        <div className="form-group fill">
                                            ...
                                            <input
                                                readOnly="readOnly"
                                                type="number"
                                                defaultValue={parseInt(itemSelected.ANO)}
                                                className='form-control '
                                    ...
                                    <Col sm={6}>
                                        <div className="form-group fill">
                                            ...
                                            <input readOnly="readOnly" type="number" defaultValue={parseInt(itemSelected.DUC_Art_48)} className="form-control" placeholder="Número do DUC ..." required />
                                    ...
                                    <Col sm={6}>
                                        <div className="form-group fill">
                                            ...
                                            <input readOnly="readOnly" type="number" defaultValue={parseInt(itemSelected.DUC_Art_49)}
                                                className="form-control" placeholder="Número do DUC ..." required />
                                    ...
                                    <Col sm={12}>
                                        <div className="form-group fill">
                                            ...
                                            <input readOnly="readOnly" type="text" defaultValue={itemSelected.BRUTO} className="form-control" placeholder="Receita Bruta..." required />
                                    ...
                                    <Col sm={6}>
                                        <div className="form-group fill">
                                            ...
                                            <input readOnly="readOnly" type="text" onChange={event => { setArt_48(event.target.value) }} defaultValue={itemSelected.Art_48_percent} className="form-control" placeholder="DUC (Art° 48)..." required />
                                    ...
                                    <Col sm={6}>
                                        <div className="form-group fill">
                                            ...
                                            <input readOnly="readOnly" type="text" onChange={event => { setArt_49(event.target.value) }} defaultValue={itemSelected.Art_49_percent} className="form-control" placeholder="DUC (Art° 49)..." required />
                                    ...
                                    <Row>
                                        <Col sm={12}>
                                            <div className="form-group fill">
                                                ...
                                                <input
                                                    readOnly="readOnly"
                                                    defaultValue={createDate2(itemSelected.pagamento[0].DT_CONFIRMACAO)}
                                                    type="date"
                                                    className="form-control"
                                                    placeholder="Data..."
                                                    required
                                        ...
                                        <Col sm={6}>
                                            <div className="form-group fill">
                                                <label className="floating-label" htmlFor="Name">
                                                    Número do DUC 1<span style={{ color: "red" }}>*</span>
                                                </label>
                                                <input
                                                    readOnly="readOnly"
                                                    defaultValue={itemSelected.pagamento[0].DUC}
                                                    type="number"
                                                    className="form-control"
                                                    placeholder="Número do DUC..."
                                        ...
                                        <Col sm={6}>
                                            ...
                                            <div className="form-group fill">
                                                <label className="floating-label" htmlFor="Name">
                                                    Data Emissão do DUC 1<span style={{ color: "red" }}>*</span>
                                                </label>
                                                <input
                                                    type="date"
                                                    max={todayDate}
                                                    defaultValue={createDate2(itemSelected.pagamento[0].DT_EMISSAO_DUC)}
                                                    readOnly="readOnly"
                                        ...
                                        <Col sm={6}>
                                            <div className="form-group fill">
                                                <label className="floating-label" htmlFor="Name">
                                                    Número do DUC 2 <span style={{ color: "red" }}>*</span>
                                                </label>
                                                <input
                                                    readOnly="readOnly"
                                                    defaultValue={itemSelected.pagamento[0].DUC_2}
                                                    type="number"
                                                    className="form-control"
                                                    placeholder="Número do DUC..."
                                        ...
                                        <Col sm={6}>
                                            ...
                                            <div className="form-group fill">
                                                <label className="floating-label" htmlFor="Name">
                                                    Data Emissão do DUC 2<span style={{ color: "red" }}>*</span>
                                                </label>
                                                <input
                                                    type="date"
                                                    max={todayDate}
                                                    defaultValue={createDate2(itemSelected.pagamento[0].DT_EMISSAO_DUC_2)}
                                                    readOnly="readOnly"
                                        ...
                                        <Col sm={12}>
                                            <div className="d-flex">
                                                {itemSelected.pagamento[0].sgigjreldocumento.length > 0 ? (
                                                    itemSelected.pagamento[0].sgigjreldocumento.map((documento, index) => (
                            ...
                            <form id="criarItem" onSubmit={criarRecibimentoGO} >
                                ...
                                <Row>
                                    ...
                                    <Col sm={6}>
                                        <div className="form-group fill">
                                            <label className="floating-label" htmlFor="Name">
                                                Número do DUC <span style={{ color: "red" }}>*</span>
                                            </label>
                                            <input
                                                onChange={(event) => {
                                                    setDUC(event.target.value);
                                                }}
                                                defaultValue={""}
                                                type="number"
                                                className="form-control"
                                                    ...
                                                    {selectedFiles.map((file, index) => (
                                                        <>
                                                            <li className='d-flex justify-content-between ' key={index}>{file.name}
                                                                <span style={{ cursor: "pointer" }} onClick={() => removenovosdocumentos(index)}
