...
const ContribuicoesTable = ({ data, loading }) => {
    if (loading) {
        return (
    ...
    if (!data || data.length === 0) {
        return (
            <div className="empty-state">
                ...
                <p>Sem dados de contribuicoes disponiveis</p>
            </div>
        );
    }
    ...
    // Pivotear: entidade -> { ano: valor_total }
    const pivoted = {};
    entidades.forEach(ent => {
        pivoted[ent] = {};
        anos.forEach(ano => {
            const row = data.find(d => d.entidade === ent && d.ano === ano);
            pivoted[ent][ano] = row ? (row.valor_total || 0) : 0;
        });
    });
...
export default ContribuicoesTable;
