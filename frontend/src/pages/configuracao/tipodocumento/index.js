...
const pageAcess = "/configuracao/tipodocumento"
...
const Customers = () => {
    ...
    const [FIELD_SELECTED, setFIELD_SELECTED] = useState(false);
    ...
    const openVerHandler = (idx) => {
        ...
        const isDocIdentification = idx.IDENTIFICACAO == 1 ? true : false
        setFIELD_SELECTED(isDocIdentification)
        setIsOpen(false);
        setitemSelected(idx)
    };
    ...
    async function editarItemGO(event) {
        ...
        const upload = {
            ...
            IDENTIFICACAO: FIELD_SELECTED ? "1" : "0"
        ...
        try {
            ...
            const response = await api.put('/sgigjprdocumentotp/' + itemSelected.ID, upload);
    ...
    async function criarItemGO(event) {
        ...
        try {
            ...
            const response = await api.post('/sgigjprdocumentotp', upload);
    ...
    const removeItemFunction = async (idx) => {
        ...
        try {
            ...
            const response = await api.delete('/sgigjprdocumentotp/' + idx);
            ...
            <Row>
                <Col sm={12}>
                        ...
                        <Modal.Body className="newuserbox" >
                            <form id="criarItem" onSubmit={criarItemGO} >
                                <Row>
                                    ...
                                    <div className="col-sm-12">
                                        <div className="form-group fill">
                                            {/* <label className="floating-label" htmlFor="Address">Documento de Identificação</label> */}
                                            <Form.Check label="Documento de Identificação" onChange={e => handleChange(e.target.checked)} />
                            ...
                            <form id="editarItem" onSubmit={editarItemGO} >
                                ...
                                <Row>
                                    ...
                                    <Col sm={8}>
                                        <div className="form-group fill">
                                            <label className="floating-label" htmlFor="Name">Designação <span style={{ color: "red" }} >*</span></label>
                                            <input maxLength="128" type="text" onChange={event => { setDESIG_C(event.target.value) }} defaultValue={itemSelected.DESIG} className="form-control" id="Utilizador" placeholder="Designação..." required />
                                        </div>
                                    </Col>
                                    ...
                                    <div className="col-sm-12">
                                        <div className="form-group fill">
                                            {/* <label className="floating-label" htmlFor="Address">Documento de Identificação</label> */}
                                            <Form.Check checked={FIELD_SELECTED} label="Documento de Identificação" onChange={e => handleChange(e.target.checked)} />
                    ...
                    <Modal size='x' show={isVerOpen} onHide={() => setVerOpen(false)}>
                        <Modal.Header closeButton>
                            <Modal.Title as="h5">Tipo de Documento</Modal.Title>
                        </Modal.Header>
                        <Modal.Body className="newuserbox" >
