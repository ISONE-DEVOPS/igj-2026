...
export default async function AboutPage() {
  // Try to load content from the Page model (slug: "sobre" or "quem-somos")
  const page = await prisma.page.findFirst({
    where: {
      slug: { in: ["sobre", "quem-somos"] },
      published: true,
    },
  });
    ...
    <>
      ...
      <section className="relative bg-gradient-to-br from-navy-900 via-navy-800 to-navy-900 text-white py-20 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4">
          ...
          <h1 className="text-4xl lg:text-5xl font-bold mb-4">{page?.title || "Quem Somos"}</h1>
          <p className="text-lg text-gray-300 max-w-2xl">
            {page?.excerpt || "Conheça a Inspecção-Geral de Jogos, a entidade reguladora de jogos e apostas de Cabo Verde."}
          </p>
        </div>
      </section>

      {/* Content from DB or fallback */}
      {page ? (
