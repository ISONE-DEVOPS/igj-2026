...
export default function PublicationFormPage() {
  ...
  const [form, setForm] = useState({
    title: "", slug: "", description: "", coverImage: "", fileUrl: "", type: "RELATORIO", published: false, publishedAt: "",
  });

  useEffect(() => {
    if (!isNew) {
      setLoading(true);
      fetch(`/api/admin/publicacoes/${params.id}`).then(r => r.json()).then(data => {
        setForm({
          title: data.title || "", slug: data.slug || "", description: data.description || "",
          coverImage: data.coverImage || "", fileUrl: data.fileUrl || "", type: data.type || "RELATORIO",
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setSaving(true);
    const url = isNew ? "/api/admin/publicacoes" : `/api/admin/publicacoes/${params.id}`;
    const res = await fetch(url, {
      method: isNew ? "POST" : "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...form, publishedAt: form.publishedAt ? new Date(form.publishedAt).toISOString() : null }),
    });
    if (res.ok) { router.push("/admin/publicacoes"); router.refresh(); } else { alert("Erro ao guardar"); }
    setSaving(false);
  }

  async function handleDelete() {
    if (!confirm("Eliminar esta publicação?")) return;
    const res = await fetch(`/api/admin/publicacoes/${params.id}`, { method: "DELETE" });
    if (res.ok) { router.push("/admin/publicacoes"); router.refresh(); }
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{isNew ? "Nova Publicação" : "Editar Publicação"}</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            <Card><CardHeader><h3 className="font-semibold text-gray-900">Detalhes</h3></CardHeader><CardBody className="space-y-4">
              <div className="space-y-1">
                ...
                <select value={form.type} onChange={(e) => setForm(f => ({ ...f, type: e.target.value }))} className="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm">
                  {publicationTypes.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
                </select>
              </div>
              <Input label="Data" type="date" value={form.publishedAt} onChange={(e) => setForm(f => ({ ...f, publishedAt: e.target.value }))} />
            ...
            <Card><CardHeader><h3 className="font-semibold text-gray-900">Imagem de Capa</h3></CardHeader><CardBody>
              <FileUpload label="Imagem" accept="image/*" value={form.coverImage} onChange={(url) => setForm(f => ({ ...f, coverImage: url }))} />
            </CardBody></Card>
            <Card><CardHeader><h3 className="font-semibold text-gray-900">Documento</h3></CardHeader><CardBody>
              <FileUpload label="Ficheiro PDF" accept=".pdf,.doc,.docx" value={form.fileUrl} onChange={(url) => setForm(f => ({ ...f, fileUrl: url }))} />
            </CardBody></Card>
